...
ID3D12Resource* createAS(
  ...
  ThrowFailedHR(getDevice()->get()->QueryInterface(&device));
...
void buildBLAS(ID3D12GraphicsCommandList4* cmdList, ID3D12Resource** blas,
               ID3D12Resource** scrach, const GPUMesh gpuMeshArr[],
               const D3D12_GPU_VIRTUAL_ADDRESS gpuTransformAddressArr[],
               UINT numMeshes, UINT vertexStride,
               D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS buildFlags) {
  D3D12_RAYTRACING_GEOMETRY_DESC commonDesc = {};
  commonDesc.Type = D3D12_RAYTRACING_GEOMETRY_TYPE_TRIANGLES;
  commonDesc.Flags = D3D12_RAYTRACING_GEOMETRY_FLAG_OPAQUE;
  commonDesc.Triangles.VertexFormat = DXGI_FORMAT_R32G32B32_FLOAT;
  commonDesc.Triangles.VertexBuffer.StrideInBytes = vertexStride;
  commonDesc.Triangles.IndexFormat = DXGI_FORMAT_R32_UINT;

  std::vector<D3D12_RAYTRACING_GEOMETRY_DESC> geoDesc(numMeshes, commonDesc);
  for (UINT i = 0; i < numMeshes; ++i) {
    geoDesc[i].Triangles.VertexCount = gpuMeshArr[i].numVertices;
    geoDesc[i].Triangles.VertexBuffer.StartAddress =
        gpuMeshArr[i].vertexBufferVA;
    geoDesc[i].Triangles.IndexCount = gpuMeshArr[i].numIndices;
    geoDesc[i].Triangles.IndexBuffer = gpuMeshArr[i].indexBufferVA;
  ...
  // Below is necessary at this point. It must be placed before getting prebuild
  // info.
...
void buildTLAS(ID3D12GraphicsCommandList4* cmdList, ID3D12Resource** tlas,
               ID3D12Resource** scrach, ID3D12Resource** instanceDescArr,
               const pID3D12Resource blasArr[],
               const dxTransform transformArr[], UINT numBlas,
               UINT instanceMultiplier,
               D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS buildFlags) {
  *instanceDescArr = createCommittedBuffer(
      (UINT64)sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * numBlas);

  D3D12_RAYTRACING_INSTANCE_DESC* pInsDescArr;
  auto range = Range(0);
  (*instanceDescArr)->Map(0, &range, reinterpret_cast<void**>(&pInsDescArr));
  {
    ...
    for (UINT i = 0; i < numBlas; ++i) {
      ...
      pInsDescArr[i].InstanceContributionToHitGroupIndex =
          i * instanceMultiplier;
      *(reinterpret_cast<dxTransform*>(pInsDescArr[i].Transform)) =
          transformArr[i];
      pInsDescArr[i].AccelerationStructure = blasArr[i]->GetGPUVirtualAddress();
    }
  }
  ...
  buildInput.Flags = buildFlags;
  // Below is not necessary at this point. It can be placed after getting
  // prebuild info.
...
MeshData::MeshData(CommandQueue* cmdQueue, CommandList* cmdList,
  ...
  if (buildAS) buildAccelerationStructure(cmdQueue, cmdList);
}

void MeshData::buildAccelerationStructure(CommandQueue* cmdQueue,
                                          CommandList* cmdList, bool onlyBlas) {
  SAFE_RELEASE(blas);
  SAFE_RELEASE(tlas);

  auto* rtCmdList =
      reinterpret_cast<ID3D12GraphicsCommandList4*>(cmdList->begin());

  ID3D12Resource* scratch[3];
  D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS buildFlags =
      D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_NONE;

  UINT vertexStride = sizeof(float) * 8;
  GPUMesh mesh;
  {
    mesh.numVertices = (UINT)vtxBuff.getBufferSize() / vertexStride;
    mesh.numIndices = (UINT)idxBuff.getBufferSize() / sizeof(UINT);
    mesh.vertexBufferVA = vtxBuff.getGpuAddress();
    mesh.indexBufferVA = idxBuff.getGpuAddress();
  }
  D3D12_GPU_VIRTUAL_ADDRESS geoTranform = 0;

  buildBLAS(rtCmdList, &blas, &scratch[0], &mesh, &geoTranform, 1, vertexStride,
            buildFlags);

  if (!onlyBlas) {
    dxTransform instTransform(modelMat);

    buildTLAS(rtCmdList, &tlas, &scratch[1], &scratch[2], &blas, &instTransform,
              1, 0, buildFlags);
  }
