...
struct dxTransform {
  float mat[3][4]{};
  dxTransform() {}
  explicit dxTransform(float scala) : mat{} {
    mat[0][0] = mat[1][1] = mat[2][2] = scala;
  }
  explicit dxTransform(const XMMATRIX& tm) { *this = tm; }
  void operator=(const Transform& tm) {
    *this = *(reinterpret_cast<const dxTransform*>(&tm));
  }
  void operator=(const XMMATRIX& tm) {
    *this = *(reinterpret_cast<const dxTransform*>(&tm));
  }
  pFloat4 data() { return mat; }
};

typedef ID3D12Resource* pID3D12Resource;
...
class MeshData {
 public:
  ...
  void buildAccelerationStructure(CommandQueue* cmdQueue, CommandList* cmdList,
                                  bool onlyBlas = false);
  const pID3D12Resource getBlas() const { return blas; }
  const pID3D12Resource getTlas() const { return tlas; }
};
